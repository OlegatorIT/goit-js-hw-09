...
const options = {
    ...
    onClose (selectedDates) {
        if (selectedDates[0] < new Date()) {
            ...
            if (btnStart.classList.contains("notActiv")) return;
...
const stopWatch = (0, _flatpickrDefault.default)("#datetime-picker", options);
...
function startTimer() {
    ...
    idInterval = setInterval(()=>{
        const currentValue = stopWatch.selectedDates[0].getTime() - new Date().getTime();
        const { days , hours , minutes , seconds  } = convertMs(currentValue);
        valueSeconds.textContent = addLeadingZero(seconds), valueMinutes.textContent = addLeadingZero(minutes), valueHours.textContent = addLeadingZero(hours), valueDays.textContent = addLeadingZero(days);
